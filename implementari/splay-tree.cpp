...
Node* splay(Node *node, Node* upper) {
    while (node->parent != upper) {
...
void erase(int x) {
    ...
    if (node->left == nullptr) {
        ...
        if (node->right == nullptr) {
            ...
            newNode = goRight(node->left);
            newNode = splay(newNode, node);
            assignRightChild(newNode, node->right);
        }
    }
    transplant(node, newNode);
    if (node->parent != nullptr) {
        root = splay(node->parent, nullptr);
    }
    delete node;
}
