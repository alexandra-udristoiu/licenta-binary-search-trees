...
Node *insertAsRoot(Node *pNode, int x);
...
Node* insert(Node* node, int x) {
    if (node == NULL) {
        ...
        if (getRandomNumber(node->size + 1) == node->size) {
            return insertAsRoot(node, x);
        }
        if (node->key > x) {
...
Node *insertAsRoot(Node* node, int x) {
    if (node == NULL) {
        return new Node(x);
    }
    else {
        if (node->key > x) {
            node->left = insertAsRoot(node->left, x);
            node = rotateRight(node);
        }
        else {
            node->right = insertAsRoot(node->right, x);
...
Node* erase(Node* node, int x) {
    if (node->key == x) {
        ...
        if (node->right == NULL || (node->left != NULL && getRandomNumber(node->size - 1) < node->left->size)) {
            node = rotateRight(node);
